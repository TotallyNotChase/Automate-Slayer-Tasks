...
import selenium.webdriver.firefox.options as option
...
def exitScript(code, message = None):
    ...
    if code == 0:
        ...
        input("ERROR (4): An unexpected error occurred, make sure that geckodriver is present in the same directory.\n    You may close this console or press any key\n")
        browser.close()
        sys.exit(4)
    elif code == 5:
...
if __name__ == "__main__":
    ...
    for item in reversed(eventlist):
        ...
        try:
            ...
            extended_wait.until(EC.presence_of_element_located((by.By.XPATH, "//*[@class = 'check check--small']"))
                           )
